...
- **`chorale review [--staged] [paths…]`** — review your `git diff` (working tree or index) or given files with the reviewer agent. The reviewer gained a diff-review mode (judge the change, catch regressions) and multi-file cross-contract review (reads files via its tools).
- **`reviewGate` agent toggle** — after a coding agent's code verifies clean, the `reviewer` gives a semantic second opinion and any BLOCKER/MAJOR loops back for a fix. Default-on for the `coder` (catches security/logic/async bugs syntax+smoke miss); `CHORALE_NO_REVIEW_GATE=1` to disable.
- **Scribe agent** (Phase 4, Task 2) — a files/docs specialist: grounded generation (README/API/CHANGELOG), summarize/extract, meaning-preserving edits, staleness detection, and applied file organization. New reference-safe **`move`** fs tool (renames/moves within the sandbox, reports links to update, never deletes) and a **`groundCheck`** anti-hallucination pass (`src/core/ground.ts`) that verifies referenced paths exist. Benchmarked (`eval/scribe-*.ts`, `SCRIBE-RESULTS.md`): on gemma, 0 invented refs · recall 2/2 · staleness 3/3 · edit-safety ✓.
- **`groundCheck` agent toggle** + **agent-agnostic self-critique** — the shared `selfCritique` prompt is no longer reviewer-specific, so it works for the scribe (and any agent) without derailing them, while keeping the reviewer un-regressed.
- **Multi-format document I/O for the scribe** — new tools `read_doc` / `write_doc` / `write_sheet` / `convert` let it read, create, and convert **PDF, DOCX, XLSX, PPTX, HTML, CSV/TSV, JSON**, and **images (OCR)**. The model authors Markdown/rows; deterministic tools do the binary I/O (exceljs, mammoth, docx via html-to-docx, pdf-parse, pdfkit, marked, pptxgenjs, officeparser, tesseract.js). PDF renders via headless Chrome/Edge with a pdfkit fallback. Round-trip benchmark: gemma 11/11. `npm audit` stays 0 (`uuid` pinned via overrides). **Presentation:** professional themes (`theme` = report/docs/minimal — cover, design tokens, styled tables, callouts, print + dark), numeric-table **bar charts** (`charts: true`), and a **bespoke design mode** where the model authors custom HTML with an automatic fidelity check that forbids inventing data (grounded polish that beats a generic converter and a source-blind design).
- **Planner/Architect agent + gates** (Phase 4, Task 3) — the `planner` decomposes a request into a grounded, validated plan (`plan.ts`: DAG + assignments + acceptance + grounding, validate-repair loop); generalized **gates** (an agent runs a permitted other agent as an advisory step, ancestor-exclusion loop guard, auto + on-demand `gate()` tool); plan-first wiring so a complex task is decomposed before it is built.
- **Test-writer agent** (Phase 4, Task 4) — writes and runs tests for existing code, mutation-graded.
- **Escalate-last system** — a **Playbook** (`{issue → verified fix}` store with intelligent trust + per-model capability profiles) and a **repair ladder** (recall known fix → delegate research → escalate to the stronger model, with verified write-back); tiered foundational-first runnability repair; a **dynamic boot gate** that installs deps and actually boots + probes the server; `chorale playbook` to inspect it.
- **Contract-first build system** (§4d) — the planner commits an **up-front interface contract** (exact endpoints, module exports, data model, deps, env) threaded verbatim into every step; a **deterministic skeleton** (`skeleton.ts` + `dependency-registry.ts`) that reconciles package.json + .env with no model call; deterministic start-script repair (`node`→`tsx`); **per-step contract-drift** detection (catches `/login` vs `/api/auth/login` mid-build); and a boot gate that launches through the real runner (tsx/ts-node) and probes the whole contract surface. `CHORALE_NO_CONTRACT=1` toggles it for A/B.
- **Free-GLM model providers** (§4e) — a `zai` provider (Z.ai's OpenAI-compatible endpoint, `https://api.z.ai/api/paas/v4`; free `glm-4.5-flash`) and a `puter` provider (`src/core/puter-provider.ts`: an OpenAI-envelope shim over `puter.ai.chat`, since Puter has no REST endpoint — with a guard for puter.js's Node WebSocket crash). Text + multi-step tool calling verified live on `glm-4.5-flash`, `glm-4.6`, `glm-5.2`. Default routing moved off gemma to `fireworks:gpt-oss-120b` (reliable) with `puter:z-ai/glm-4.6` as an opportunistic free fallback — Puter's free tier is a ~25M-unit **monthly** allowance (≈ 1 build/month).
