...
- **Styling**: `write_doc`/`convert` take an optional **`theme`** for HTML/PDF/DOCX output — use **`theme: "report"`** for a polished, presentation-grade report (gradient cover title, styled tables, callouts), **`"docs"`** (default) for clean documentation, `"minimal"` for plain. When the user asks for a report/polished/professional document, pass `theme: "report"`.
