...
| 2 ✅ | **Files / Docs specialist** (`scribe`) | **Full-scope, all-green on gemma** — grounded gen (README/API/CHANGELOG/docstrings/architecture/scaffolding), summarize/extract/synthesize, proofread/format/TOC/tone edits, staleness+sync-apply+consistency, reference-safe move+reorganize+inventory+naming, grounded Q&A (22/22). **Plus real document formats** — read·create·convert **PDF/DOCX/XLSX/PPTX/HTML/CSV + image-OCR** (11/11 round-trip; `read_doc`/`write_doc`/`write_sheet`/`convert`). `groundCheck` (paths+symbols+scripts) + intent-aware meaning-preservation. See [`eval/SCRIBE-RESULTS.md`](../eval/SCRIBE-RESULTS.md). |
| 3 ✅ | **Planner / Architect** | Decomposes a request into a grounded, **validate-repaired** plan; a generalized composable **gate** framework (ancestor-exclusion loop guard, auto/on-demand, graceful degradation); wired **plan-first** into the orchestrator. Plus fullstack levers (plan-execution-across-turns, shared contract, runnability gate, escalation) — see [`FULLSTACK-EXPERIMENT.md`](FULLSTACK-EXPERIMENT.md). |
| 4 ✅ | **Test-writer** | Writes and **runs** tests, graded by **mutation** (pass on correct code, fail on planted bugs; never weaken a test to go green). See [`eval/TESTWRITER-RESULTS.md`](../eval/TESTWRITER-RESULTS.md). |
