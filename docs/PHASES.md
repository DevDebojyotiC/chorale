...
- **Benchmark** (`eval/scribe-*.ts`, `SCRIBE-RESULTS.md`): groundedness (precision = invented refs, recall = documents the real API, with a `groundCheck` A/B), staleness detection (planted renamed-symbol/dead-link/wrong-version), edit safety (fix grammar, preserve facts). gemma: **0 invented · recall 2/2 · staleness 3/3 · edit ✓**.
